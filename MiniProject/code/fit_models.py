...
def safe_log(x: np.ndarray, min_value: float = 1e-8) -> np.ndarray:
    """Apply a log transform while avoiding zeros and negative values."""
...
def estimate_initial_params(time: np.ndarray, pop: np.ndarray) -> Dict[str, Dict[str, float]]:
    """
    Generate reasonable starting values for non-linear optimisation.

    These starting values are based on the observed data range and
    a crude estimate of early growth from the log-transformed response.
    """
...
def fit_model(model_name: str, time: np.ndarray, pop: np.ndarray) -> Tuple[Optional[np.ndarray], Dict[str, object]]:
    """
    Fit one candidate model to one curve.

    Returns:
    - fitted parameters if optimisation succeeds
    - a metrics dictionary recording fit quality and convergence
    """
    ...
    try:
        if model_name == "logistic":
            ...
            params, _ = curve_fit(logistic_model, time, pop, p0=p0, bounds=bounds, maxfev=20000)
            ...
            params, _ = curve_fit(gompertz_model, time, pop, p0=p0, bounds=bounds, maxfev=20000)
...
def fit_single_curve(curve_df: pd.DataFrame) -> Tuple[List[Dict], List[Dict]]:
    """
    Fit both models to a single growth curve.

    Produces:
    - one parameter record per successful fit
    - one metrics record per fit attempt
    """
    ...
    static_info = {
        ...
        "Time_units": curve_df["Time_units"].iloc[0],
        "PopBio_units": curve_df["PopBio_units"].iloc[0],
    }
    ...
    for model_name in ["logistic", "gompertz"]:
        ...
        metric_records.append({**static_info, **metrics})
...
def main() -> None:
    """Fit logistic and Gompertz models to every curve in the cleaned dataset."""
    ...
    # Save outputs for model comparison and plotting
